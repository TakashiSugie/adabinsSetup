...
def eval(
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(sequential):
            ...
            if args.save_dir is not None:
                if args.dataset == "nyu":
                    impath = f"{batch['image_path'][0].replace('/', '__').replace('.png', '')}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gt_path", default="../dataset/nyu/sync/", type=str, help="path to dataset gt"
    )
    ...
    parser.add_argument(
        "--do_kb_crop", help="Use kitti benchmark cropping", action="store_true"
    )
